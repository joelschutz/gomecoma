...
// The MovieGenreFunc type is an adapter to allow the use of ordinary
// function as MovieGenre mutator.
type MovieGenreFunc func(context.Context, *ent.MovieGenreMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MovieGenreFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.MovieGenreMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MovieGenreMutation", m)
	}
	return f(ctx, mv)
}
