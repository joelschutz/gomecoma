...
func (fq *FileQuery) QueryMovie() *MovieQuery {
	query := &MovieQuery{config: fq.config}
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withMovie; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*File)
		for i := range nodes {
			if nodes[i].movie_file == nil {
				continue
			}
			fk := *nodes[i].movie_file
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "movie_file" returned %v`, n.ID)
			}
			for i := range nodes {
